        ...
        public override List<string> Process(string input, ContentProcessorContext context)
        {
            ...
            using (FileStream fileStream = new FileStream(input, FileMode.Create, FileAccess.Write))
            {
                using (StreamWriter writer = new StreamWriter(fileStream))
                {
                    ...
                    {
                        long sna_file_size = 0;

                        try
                        {
                            FileInfo f_info = new FileInfo(contentDirectory + "\\" + file + ".sna" );
                            sna_file_size = f_info.Length;
                        }
                        catch( Exception )
                        {
                        ...
                        {
                            writer.WriteLine(file + "," + sna_file_size);
                            entries.Add(file + "," + sna_file_size);
                        }
                    }
                }
            }
